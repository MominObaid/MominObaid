# 👋 Hi, I'm Momin Obaid 

💻 **Mobile App (Android) Developer**  
⚡ Electronics Engineer transitioning into **Software Development**

I enjoy building **mobile applications that solve real-world problems** and creating clean, intuitive user experiences.  
My focus is on developing **scalable Android applications** while continuously improving my skills in modern mobile technologies.

---

## 🚀 Tech Stack

### 📱 Mobile Development
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white)
![Android_native](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white)
![XML](https://img.shields.io/badge/XML-E44D26?style=for-the-badge&logo=w3c&logoColor=white)
![React Native](https://img.shields.io/badge/React_Native-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)

### 💻 Programming
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

### 🛠 Tools & Version Control
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

---

## 🌱 Currently Learning

- Advanced **Android Architecture (MVVM)**
- **Clean Architecture** for scalable apps
- **Jetpack Libraries**
- **Jetpack Compose**
- Mobile app performance optimization
- UI/UX best practices for modern apps

---

## 📱 Featured Projects

### 🏋️ GYMLOG -- Workout Tracker App
A mobile application that helps users track workouts, manage fitness routines, and monitor progress.

**Tech Used**
- Kotlin
- Android SDK
- XML
- Local Database

---

### 💰 Expense Manager App
An application to track daily expenses and manage personal finances effectively.

**Tech Used**
- React Native
- JavaScript
- Local Storage

---

### 📋 Task Manager App
A productivity app to organize tasks, track progress, and manage daily goals.

**Tech Used**
- Kotlin
- Android
- XML

---

### 🏃 Kinetic Fitness (Internship Project)
A **React Native mobile application** developed during my internship, focused on helping users manage fitness routines and stay active.

**Tech Used**
- React Native
- JavaScript
- Mobile UI Components

---



## 📫 Connect With Me

💼 LinkedIn: [https://linkedin.com/in/YOUR_LINKEDIN  ](https://www.linkedin.com/in/momin-obaid-344769103/)

📧 Email: obaidurrahmanobaid@gmail.com

---

⭐ **"Building mobile experiences, one line of code at a time."**
